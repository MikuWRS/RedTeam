...
def main():
	...
	if(argumentos > 3 or argumentos < 2):
		usage(sys.argv[0])
		sys.exit(1)
	elif('-host' not in sys.argv[1:] and '-port' not in sys.argv[1:] and '-snif' not in sys.argv[1:] and '-enviar' not in sys.argv[1:] and '-v' not in sys.argv[1:]):
		usage(sys.argv[0])
		sys.exit(1)
	else:
...
if __name__ == '__main__':
	main()
	#print("Valor de verbose ", verbose, " y enviar ",enviar,"\n")
